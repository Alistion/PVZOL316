...
from dal import get_arena_lineup, get_or_create_user, get_user_organisms, get_user_tools
from services import FriendService, TreeService


def build_user_xml(username):
    user = get_or_create_user(username)
    uid = user["id"] + UID_OFFSET

    money = user.get("money", 0)
    rmb = user.get("rmb_money", 0)
    level = user.get("level", 100)
    t_height = user.get("tree_height", 0)
    honor_val = user.get("honor", 0)
    ...
    <root>
        ...
        <user id="{uid}" name="{user["username"]}" charm="999" money="{money}" date_award="1" reward_daily="1" has_reward_cus="1" has_reward_once="1" has_reward_sum="1" has_reward_first="1" state="" rmb_money="{rmb}" open_cave_grid="16" wins="999" is_new="" login_reward="1" invite_amount="0" use_invite_num="0" max_use_invite_num="10" lottery_key="99" banner_num="0" banner_url="/pvz/events/" hasActivitys="1" face_url="" face="{avatar_val}" vip_grade="2" vip_etime="1893427200" vip_restore_hp="1" is_auto="0" serverbattle_status="3" IsNewTaskSystem="1" registrationReward="1" stone_cha_count="0" vip_exp="1111" >
...
def build_warehouse_xml(username):
    ...
    tools_xml = "".join(
        [
            f'<item id="{t["tool_id"]}" amount="{t["amount"]}" />'
            for t in tools
            if t["amount"] > 0 and t["tool_id"] not in hidden_tool_ids
        ]
    )
    orgs_xml = ""
    for o in orgs:
        try:
            ...
            ssk_xml = "".join(
                [
                    f'<item id="{s["id"]}" name="{s["name"]}" grade="{s["grade"]}" type="{s["type"]}" />'
                    for s in d.get("ssk", [])
                ]
            )
            tals_xml = "".join(
                [
                    f'<tal id="{t["id"]}" level="{t["level"]}" />'
                    for t in d.get("tals", [])
                ]
            )
